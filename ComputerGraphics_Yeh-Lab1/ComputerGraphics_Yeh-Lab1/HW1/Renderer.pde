public class ShapeRenderer{
    ...
    //color
    // 將變數名稱從 color 改為 selectedColor
    private Color shapeColor = Color.BLACK; // 默認顏色為黑色
    ...
    public void run(){      
      fill(shapeColor.getRGB()); // 使用當前顏色進行繪製
    ...
    public void addShape(Shape s){
      s.setColor(shapeColor); // 當前顏色應用到形狀上
      shapes.add(s);
    }   
    ...
    public void clear(){
     ...
     // 將 setColor 的參數名改為 selectedColor
    public void setColor(Color selectedColor) {
        shapeColor = selectedColor; // 設置選擇的顏色
    }
}
